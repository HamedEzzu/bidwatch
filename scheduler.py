...
def run_cycle(mode: str, tag: str, dry_run: bool) -> None:
    ...
    if mode == "agent" and not dry_run:
        ...
        stats = bidwatch.run_pipeline(tag=tag, dry_run=dry_run)
        logger.info(
...
def main() -> int:
    ...
    if args.dry_run or args.once:
        run_cycle(mode=args.mode, tag=args.tag, dry_run=args.dry_run)
        return 0
    ...
    try:
        while True:
            run_cycle(mode=args.mode, tag=args.tag, dry_run=False)
